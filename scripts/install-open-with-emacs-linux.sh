...
EOF
	...
	systemctl --user import-environment \
		PATH XDG_DATA_DIRS XDG_CONFIG_HOME XDG_DATA_HOME XDG_RUNTIME_DIR \
		DISPLAY WAYLAND_DISPLAY XAUTHORITY DBUS_SESSION_BUS_ADDRESS \
		XDG_CURRENT_DESKTOP XDG_SESSION_TYPE DESKTOP_SESSION || true

	# Re-enable to migrate older installs from default.target, then restart so an
	# already-running daemon receives the freshly imported graphical environment.
	if systemctl --user daemon-reload &&
		systemctl --user reenable emacs.service &&
		systemctl --user restart emacs.service; then
		echo "Installed, enabled, and restarted user service: $service_target"
	else
